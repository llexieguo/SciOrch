...
class SwiftMixin:
    ...
    def _rotate_flash_checkpoints(self, use_mtime=False, output_dir=None) -> None:
        ...
        # Without Flash/DLRover, `dlrover_latest.txt` is often missing and `_get_last_checkpoint_step`
        # returns 0. Then every real checkpoint has step > 0 and `valid_checkpoints` becomes empty,
        # so nothing is ever deleted and SAVE_TOTAL_LIMIT appears broken.
        if last_step == 0 and checkpoints_sorted:
            steps_found: list[int] = []
            for path in checkpoints_sorted:
                regex_match = re.match(f'.*{PREFIX_CHECKPOINT_DIR}-([0-9]+)', path)
                if regex_match is not None and regex_match.groups() is not None:
                    steps_found.append(int(regex_match.groups()[0]))
            if steps_found:
                last_step = max(steps_found)
