...
class FlexibleAccuracy(ORM):
    """Extract answer letter from various formats and compare with solution."""

    def _extract_letter(self, text):
        import re as _re
        m = _re.search(r'<answer>\s*([A-J])\s*</answer>', text)
        if m: return m.group(1)
        m = _re.search(r'\\boxed\{([A-J])\}', text)
        if m: return m.group(1)
        m = _re.search(r'(?:answer|Answer)\s*(?:is|:)\s*\*?\*?\s*\(?([A-J])\)?', text)
        if m: return m.group(1)
        m = _re.search(r'(?:correct|best)\s+(?:answer|option|choice)\s+is\s+\(?([A-J])\)?', text)
        if m: return m.group(1)
        letters = _re.findall(r'\b([A-J])\b(?:\.|,|\s|$)', text[-200:])
    ...
    def __call__(self, completions, solution, **kwargs):
        ...
        for content, sol in zip(completions, solution):
            gold = None
            m = _re.search(r'<answer>\s*([A-J])\s*</answer>', sol)
            if m: gold = m.group(1)
            else: gold = sol.strip()
            pred = self._extract_letter(content)
...
orms = {
    ...
    'flexible_accuracy': FlexibleAccuracy,
}
