...
def save_checkpoint(model: Optional[PreTrainedModel],
    ...
    # Always force tokenizer_config.json from the base model/checkpoint source
    # to avoid occasional malformed serializer output during frequent saves.
    tokenizer_tgt = os.path.join(output_dir, 'tokenizer_config.json')
    for model_dir in model_dirs:
        tokenizer_src = os.path.join(model_dir, 'tokenizer_config.json')
        if os.path.isfile(tokenizer_src):
            shutil.copy(tokenizer_src, tokenizer_tgt)
            break
