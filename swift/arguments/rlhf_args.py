...
class OfflineReinforceArguments:
    ...
    Args:
        ...
        offline_reinforce_kl_estimator (str): KL estimator variant. 'k1' = logπ - logπ_ref (original
            ...
            and the penalty is exp(Δ) - Δ - 1 once per sequence. 'ppo_clip' is a legacy alias for
            ``offline_reinforce_pg_loss_type='ppo_clip'`` plus a k1 KL term — kept for backward
            compatibility. To use PPO-clip together with k3/gspo KL, set ``offline_reinforce_pg_loss_type=ppo_clip``
            and ``offline_reinforce_kl_estimator=k3`` (or ``gspo``) explicitly.
            Defaults to 'k1' for backward compatibility.
        offline_reinforce_cliprange (float): Clipping range used when the policy gradient uses the
            PPO-clip form (``offline_reinforce_pg_loss_type=ppo_clip`` or the legacy
            ``offline_reinforce_kl_estimator=ppo_clip``). Defaults to 0.2.
        offline_reinforce_pg_loss_type (str): Policy gradient form. 'auto' = derive from
            ``offline_reinforce_kl_estimator`` for backward compatibility (i.e. ppo_clip iff
            kl_estimator='ppo_clip'). 'vanilla' = pure REINFORCE++ ``-A * logπ``. 'ppo_clip' =
            PPO-style clipped-ratio objective. Combine with any ``offline_reinforce_kl_estimator``
            (k1/k3/gspo) to mix and match — e.g. PPO-clip + k3 KL. Defaults to 'auto'.
    """
    ...
    offline_reinforce_cliprange: float = 0.2
    offline_reinforce_pg_loss_type: str = 'auto'
...
class RLHFArguments(TeacherModelArguments, GRPOArguments, PPOArguments, OfflinePPOArguments,
    ...
    def _prepare_training_args(self, training_args: Dict[str, Any]) -> None:
        ...
        if self.rlhf_type == 'offline_reinforce':
            ...
            training_args['cliprange'] = self.offline_reinforce_cliprange
            training_args['pg_loss_type'] = self.offline_reinforce_pg_loss_type
